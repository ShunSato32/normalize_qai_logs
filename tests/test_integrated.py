...
def test_integrated_no_feedback():
    ...
    assert len(rows) == 1
    row = rows[0]
    assert row["output_row_id"] == "c1-m1-FB000"
...
def test_integrated_multiple_feedbacks():
    ...
    row1 = rows[0]
    assert row1["output_row_id"] == "c1-m1-FB001"
    ...
    row2 = rows[1]
    assert row2["output_row_id"] == "c1-m1-FB002"
    ...
    # For subsequent rows, base interaction details are cleared, and 質問内容 is set to [追加評価]
    assert row1["user_content_raw"] == "Q"
    assert row2["user_content_raw"] == ""
    assert row1["質問内容"] == "Q"
    assert row2["質問内容"] == "[追加評価]"
    assert row1["回答内容"] == "A"
    assert row2["回答内容"] == ""
