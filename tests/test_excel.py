...
def test_excel_export_and_formatting(tmp_path):
    ...
    mock_rows = [
        {
            "output_row_id": "CONV001-MSG001-FB001",
            ...
            "is_system_command": 0,
            "started_at_utc": "2026-06-02T12:00:00+00:00",
            "completed_at_utc": "2026-06-02T12:00:02+00:00",
            ...
            "feedback_at_utc": "2026-06-02T12:05:00+00:00",
    ...
    assert len(row3_values) == len(HEADERS), "Headers length mismatch!"
    ...
    # IDs columns are Col B (output_row_id), Col D (interaction_id), Col E (message_id), Col F (conversation_id), Col G (reset_session_id)
    id_cols = [2, 4, 5, 6, 7]
    for col in id_cols:
    ...
    # started_at_jst is Column 21 (Col U)
    cell_dt = ws_out.cell(row=4, column=21)
    import datetime
    ...
    # score① is Column 43 (Col AQ)
    cell_score = ws_out.cell(row=4, column=43)
    ...
    assert p.cache.cacheSource.worksheetSource.ref == f"B3:DG4", "Pivot Table cache ref range was not updated!"
...
def test_custom_column_config(tmp_path):
    ...
    config_file_path = os.path.join(script_dir, "column_config.json")
