...
※ 共有先の運用PCに導入されているファイルセキュリティ（InfoCage FileShell 等）による暗号化の影響で、エクセルテンプレートファイルが正常にロードできない（`BadZipFile`等のエラーでクラッシュする）場合は、以下のコマンドでテンプレートを使わずにプログラム側でワークブックをゼロから生成するモードで実行可能です：
```bash
python run_pipeline.py --no-template
# または直接スクリプトを実行する際に引数を指定:
python excel_generator/normalize_chatbot_logs.py --no-template
```
...
- **`config/common_config.json`**: 共通システム設定を記述します。
  - `excluded_user`: 除外したいユーザー名 (`user_name`) の配列。ここに指定されたユーザーの対話データはエクセルや各出力ファイルに含まれません（例: `["管理者A", "テストユーザー"]`）。
  - `file_server`: 定期実行（WindowsタスクスケジューラやCronなど）における、コピー先の社内ファイルサーバーのパスなどの設定。デフォルトでは無効化されています。
